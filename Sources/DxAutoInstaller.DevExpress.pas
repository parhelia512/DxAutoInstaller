...
class function TDevExpressManifest.CustomFileName: string;
begin
  Result := TPath.Combine(ExtractFilePath(Application.ExeName), CustomFileBaseName);
end;

class function TDevExpressManifest.CustomFileExists: Boolean;
begin
  Result := FileExists(CustomFileName);
end;

constructor TDevExpressManifest.Create(const AUseCustomFile: Boolean);
begin
  FIsCustom := AUseCustomFile and CustomFileExists;
  ReadComponents;
end;

procedure TDevExpressManifest.ReadComponents;
const
...
procedure TDevExpressManifest.Export;
begin
